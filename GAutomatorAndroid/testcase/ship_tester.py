...
from game_lib import cheat_lib, sence_lib, ui_lib
...
def empty(ship):
    if not engine_find_elements_path(ship.object_name + "/*"):
        return True
    return False
...
def scroll_down():
    ...
    """
    ui_lib.scroll_down("##scroll_shipiconlist")
    time.sleep(1)
    # 一次拉了3行，每行4个，新增12个单位
    return 3 * 4
...
def calc_ship_num():
    ship_num_layer = engine_find_element("##ship_bag_limit_text")
    ship_num_str = engine_get_element_text(ship_num_layer)
...
def ensure_clickable(r):
    ...
    :param r:
    """
    if ui_lib.in_scroll("##scroll_shipiconlist", r):
        pass
    else:
        scroll_down()
        time.sleep(3)
...
def calc_ship_star(path_pre):
    ...
    """
    # 这里是界面就写反了，不是这里代码写错了
    silver_star_lable = engine_find_elements_path(
        path_pre + "/Object-golden-stars/*")
    gold_star_lable = engine_find_elements_path(
...
def do_one_ship_search(typeid, star=0, level=0):
    ...
    ship_list = engine_find_elements_path("/UI Root/window_ship_content/##scroll_shipiconlist/content/*")
    for ship in ship_list:
        img = engine_find_element(ship.object_name + "/obj_ship_icon(Clone)/##shipNormal/##img_icon")
        if not img:
            continue
        if engine_get_element_image(img) != ship_img_str:
            continue
        if star > 0:
            ...
            if star != ship_star:
                continue
        if level > 0:
            level_lable = engine_find_element(
                ship.object_name + "/obj_ship_icon(Clone)/##shipNormal/##level")
            ship_level = int(engine_get_element_text(level_lable))
            if level != ship_level:
                continue
        return ship
    return None
...
def get_cur_level():
    ...
    level_str = engine_get_element_text(level_lable)
...
def ensure_max_ship():
    sence_lib.enter_ship_main()
    buy_btn = engine_find_element("##obj_listbuy_btn")
    engine_click(buy_btn)
    ok_btn = engine_find_element("##Button_sure")
...
def deal_need(need_list):
    for need in need_list:
        num_layer = (engine_find_elements_path(need.object_name + "/numbers/*"))[0]
        m = re.match(r"\d+/(\d)", engine_get_element_text(num_layer))
        assert m, "没有匹配到需要的数量"
        need_num = int(m.group(1))
        engine_click(need)
        ships = engine_find_elements_path("/UI Root/obj_shiplist_middle/##scorll_partion/content/*")
        assert len(ships) >= need_num, "待选材料不足"
        for i in range(0, need_num):
            engine_click(ships[i])
        assert ui_lib.find_then_click("##choose_btn")


def do_one_up_star():
    need_list = engine_find_elements_path("/UI Root/window_neo_shipyard/adapter/FuncNodes/##node_upstar"
                                          "/obj_ship_upstars/##can_up_star/Object-bottomlist/*")
    deal_need(need_list)
    assert ui_lib.find_then_click("##upstar_btn")
    sence_lib.clear_blank_close()
...
def test_up_star(typeid):
    ...
    assert ui_lib.find_then_click("##Toggle5"), "没有找到改造按钮"
    time.sleep(1)
