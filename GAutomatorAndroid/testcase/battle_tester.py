...
def start_game():
    start_button = engine_find_element("##btn_login")
    engine_click(start_button)
...
def clear_main():
    ...
    close_button = engine_find_element("##btnClose")
    while close_button is not None:
        close_button = engine_find_element("##btnClose")
        engine_click(close_button)


def enter_battle():
    battle = engine_find_element("##btn_battle")
    engine_click(battle)


def hang_on_battle():
    battle1 = engine_find_element("/UI Root/window_fight_main/GameObject ("
                                  "1)/##node_dungeonlist/obj_dungeon_list1/##viewport/##content/##dungeon1")
    engine_click(battle1)
    find_element_wait("#g#node_drop_fixed")
    forward_btn = engine_find_element("##btn_forward")
    if forward_btn is not None:
        engine_click(forward_btn)
    else:
        close_button = engine_find_element("##btnClose")
        engine_click(close_button)
...
def clear_story():
    story_layer = engine_find_element("##btn_next")
    if story_layer is None:
        return False
    else:
        while story_layer is not None:
            story_layer = engine_find_element("##btn_next")
            engine_click(story_layer)
            time.sleep(1)
...
def do_hang_on():
    ...
    enter_btn = engine_find_element("##next")
    engine_click(enter_btn)
    find_element_wait("#g#node_drop_fixed")
    forward_btn = engine_find_element("##btn_forward")
    if forward_btn is not None:
        engine_click(forward_btn)
...
def handle_do_one_fight():
    ...
    """
    fight_btn = engine_find_element("##btn_fight")
